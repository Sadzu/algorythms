...
private:
    ...
    // массивы: ключей, значений и состояний
    Key* keyArray;
    Data* dataArray;
    State* stateArray;
    ...
    int size;
    // количество просмотренных ячеек в последней операции
    int looked;
    // индекс последней операции (i = h(k’))
    int lastIndex;
    // последний исходный ключ
    Key lastKey;
    // последний преобразованный ключ
    unsigned long long lastHashKey;

    static const unsigned int MERSENNE_PRIMES[11];

public:
    // конструктор
    HashTable(int cap = 127);
    // деструктор
    ~HashTable();

    // вставка по ключу
    bool insertData(Key key, Data data);
    // удаление по ключу
    bool deleteData(Key key);
    // поиск по ключу
    ...
    // хеш-функция для преобразования ключа
    int hash(Key key);

    // информация о последней операции
    std::pair<Key, unsigned long long> getLastKeyInfo();  // исходный и преобразованный ключи
    int getLastIndex();                                     // индекс h(k’)
    int getLooked();                                        // число зондирований
    ...
    public:
        Iterator(HashTable<Key, Data>* t, int pos = -1);

        Data& operator*();                  // доступ к текущему элементу
        Iterator& operator++();            // переход к следующему
    ...
    Iterator begin();   // установленный итератор
    Iterator end();     // неустановленный итератор

private:
    // хеш-функции
    int hash2(Key key); // для двойного хэширования, результат никогда не должен быть 0
    unsigned long long stringToInt(const std::string& key); // конкатенация битовых образов символов

    // Универсальная функция для преобразования ключа в строку
    template <typename T>
